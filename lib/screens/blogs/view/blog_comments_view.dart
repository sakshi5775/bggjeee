...
class BlogCommentsView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: LocalizedText(text: AppTranslations.Translations.allComments),
      ...
      body: Padding(
        ...
        child: Obx(() {
          ...
          if (controller.comments.isEmpty) {
            return Center(
              child: LocalizedText(text: AppTranslations.Translations.noCommentsFound, style: MyTextTheme.smallBCN.copyWith(color: '#3E2723'.toColor().withOpacity(0.7))),
            );
          }
          return ListView.separated(
            itemCount: controller.comments.length,
            separatorBuilder: (_, __) => Spacing.h(12),
            itemBuilder: (_, i) => CommentItem(comment: controller.comments[i]),
          );
        }),
      ),
    );
