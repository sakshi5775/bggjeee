...
class SunSignWidget extends StatelessWidget {
  final HoroscopeMainController controller;

  const SunSignWidget({super.key, required this.controller});

  @override
  Widget build(BuildContext context) {
    return Obx(() {
      if (controller.isLoadingSunSign.value) {
        return Center(
          child: Column(
      ...
      final data = controller.sunSignData.value;
      if (data == null || data.isEmpty) {
        return Center(
          child: AutoTranslateText(
            'No Sun Sign data available',
            style: MyTextTheme.mediumBCN.copyWith(
              color: AppColors.textSecondary,
            ),
          ),
