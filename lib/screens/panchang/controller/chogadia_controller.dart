    ...
    try {
      ...
      if (data != null) {
        ...
        if (response != null) {
          ...
          if (dayList != null) {
            dayChogadias.value = dayList
                .map((c) => c as Map<String, dynamic>)
                .toList();
          }

          if (nightList != null) {
            nightChogadias.value = nightList
                .map((c) => c as Map<String, dynamic>)
                .toList();
          }
          ...
          if (kDebugMode) {
            debugPrint(
              'Chogadia data fetched successfully: ${allChogadias.length} chogadias (${dayChogadias.length} day, ${nightChogadias.length} night)',
            );
          }
        } else {
          if (kDebugMode) {
        ...
        showErrorMessage(
          title: 'Error',
          message: 'Failed to fetch chogadia data',
        );
      }
    } catch (e) {
