...
class HinduCalendarMonthlyPanchangController extends BaseController {
  ...
  Future<void> fetchMonthlyPanchang() async {
    ...
    try {
      ...
      monthlyPanchangData.clear();
      festivalsByMonth.clear();

      // Fetch panchang data for selected date only
      ...
      if (panchangData != null && panchangData['response'] != null) {
        final response = panchangData['response'] as List<dynamic>?;
        if (response != null && response.isNotEmpty) {
          ...
          for (var item in response) {
            ...
            if (apiDateStr.isNotEmpty) {
              try {
                // Parse date from "2025/12/19" format
                ...
                // Fallback: extract day number from selected date
                dayData['dayNumber'] = selectedDate.value.day;
                dayData['date'] = apiDateStr;
              }
            } else {
