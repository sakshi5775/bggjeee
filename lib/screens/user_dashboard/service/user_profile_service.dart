...
class UserProfileService with ApiHelperMixin {
  ...
  Future<UserProfileModel?> getProfile([String? userId]) async {
    try {
      // Try fetching current profile first (usually uses Bearer token)
      ...
      print("Error fetching generic profile: $e");
    }

    // Fallback: If userId is provided, try fetching specifically for that user
    if (userId != null && userId.isNotEmpty) {
      try {
        ...
        print("Error fetching profile by userId ($userId): $e");
        rethrow;
      }
    }
