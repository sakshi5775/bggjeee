...
class AstrologerCacheService {
  ...
  static const String _astrologersCacheTimestampKey = 'astrologers_cache_timestamp';
  
  // Cache duration: 5 minutes
  static const Duration _cacheDuration = Duration(minutes: 5);
  
  /// Save astrologers to cache
  static Future<void> saveAstrologers(AstrologerResponse response, {Map<String, dynamic>? rawJson}) async {
    try {
      // Store the raw JSON if provided, otherwise construct it
      final jsonData = rawJson ?? {
        'success': true,
        'data': {
          'astrologers': response.astrologers.map((a) => _astrologerToJson(a)).toList(),
          'pagination': {
      ...
      await _storage.write(_astrologersCacheTimestampKey, DateTime.now().toIso8601String());
    } catch (e) {
  ...
  static Future<void> saveLiveStreams(LiveStreamResponse response, {Map<String, dynamic>? rawJson}) async {
    try {
  ...
  static Future<void> saveUpcomingStreams(UpcomingStreamsResponse response, {Map<String, dynamic>? rawJson}) async {
    try {
      // Store the raw JSON if provided
      final jsonData = rawJson ?? {
        'success': true,
        'data': response.streams.map((s) => _upcomingStreamToJson(s)).toList(),
        'pagination': {
          'page': response.pagination.page,
          'limit': response.pagination.limit,
          'total': response.pagination.total,
  ...
  static Map<String, dynamic> _upcomingStreamToJson(UpcomingStreamModel stream) {
    return {
      'streamId': stream.streamId,
      'astrologerId': stream.astrologerId,
      'astrologerName': stream.astrologerName,
      'scheduling': {
        ...
        'scheduledStartTime': stream.scheduling.scheduledStartTime.toIso8601String(),
        'scheduledEndTime': stream.scheduling.scheduledEndTime.toIso8601String(),
